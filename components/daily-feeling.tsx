...
import { saveDailyMood } from "@/lib/actions"
...
export function DailyFeeling({ onSubmit }: DailyFeelingProps) {
  ...
  const handleSubmit = async () => {
    if (selectedMood !== null) {
      ...
      const result = await saveDailyMood(moodData)

      if (result.success) {
        onSubmit(moodData)
        setIsSubmitted(true)
      } else {
        alert("Неуспешно зачувано расположение. Ве молиме обидете се повторно.")
      }
      ...
      <div className="max-w-2xl mx-auto">
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <h2 className="text-2xl font-bold mb-2">Вашето расположение е зачувано!</h2>
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Одбери го емотиконот што најдобро го претставува твоето чувство во моментов. </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Што придонесе да се чуствуваш вака денес? Некој специфичен настан или момент?"
              className="min-h-[100px]"
            />
          </div>

          <Button onClick={handleSubmit} disabled={selectedMood === null || isSubmitting} className="w-full">
            {isSubmitting ? "SСе зачувува..." : "Зачувај го моето расположение"}
          </Button>
        </CardContent>
      </Card>
          ...
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-lg font-medium">
                Се чуствуваш {moods.find((m) => m.value === selectedMood)?.label.toLowerCase()} денес
              </p>
              <p className="text-sm text-muted-foreground mt-2">
                Не заборавај, во ред е да имаш различни чуства. Секоја емоција е валидна и ние сме со тебе!.
              </p>
            </div>
          </CardContent>
        </Card>
